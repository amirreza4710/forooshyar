...
**وضعیت:** (ب) ستون `deleted_at` وجود نداره و تست‌ها fail می‌شن.
**پیام خطای دقیق:** `error: column "deleted_at" of relation "users" does not exist`
طبق اجرای تست `artifacts/api-server/src/routes/soft-delete.test.ts` با vitest، دیتابیس فعلی فاقد ستون `deleted_at` برای جدول‌ها است و به همین دلیل تست‌های مربوط به soft delete با خطای بالا شکست خوردند.
**پیشنهاد قدم بعدی:** لطفاً تأیید صریح بفرمایید تا من دستور `pnpm --filter @workspace/db push` را روی دیتابیس dev اجرا کنم و یا خودتان این دستور را اجرا نمایید.
